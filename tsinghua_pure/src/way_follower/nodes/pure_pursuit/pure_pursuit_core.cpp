...
PurePursuitNode::PurePursuitNode()
  ...
  , param_flag_(-1)
  , const_lookahead_distance_(2)//4
  , const_velocity_(5.0)
  , lookahead_distance_ratio_(2.0)
  , minimum_lookahead_distance_(3)//6
{
  initForROS();
...
void PurePursuitNode::initForROS()
{
  ...
  nh_.param("vehicle_info/wheel_base", wheel_base_, double(1.5));
  ...
  //pub1_ = nh_.advertise<geometry_msgs::TwistStamped>("cmd_vel", 10);
  pub2_ = nh_.advertise<autoware_msgs::ControlCommandStamped>("ctrl_cmd", 10);
  //pub2_ = nh_.advertise<ackermann_msgs::AckermannDriveStamped>("ackermann_cmd", 10);
...
void PurePursuitNode::run()
{
  ...
  while (ros::ok())
  {
    ...
    ROS_INFO_STREAM("lookaheaddistance_well ");
    pp_.setMinimumLookaheadDistance(minimum_lookahead_distance_);
    ROS_INFO_STREAM("minmum_lookahead_distance_well ");
...
void PurePursuitNode::publishTwistStamped(const bool &can_get_curvature, const double &kappa) const
{
  ...
  ts.twist.linear.x = can_get_curvature ? computeCommandVelocity(kappa) : 0;
  //ts.twist.angular.z = can_get_curvature ? kappa * ts.twist.linear.x : 0;
  ts.twist.angular.z = can_get_curvature ? atan(wheel_base_ * kappa) : 0;
...
void PurePursuitNode::publishControlCommandStamped(const bool &can_get_curvature, const double &kappa) const
{
  ...
  ccs.cmd.linear_velocity = can_get_curvature ? computeCommandVelocity(kappa) : 0;
  //ccs.cmd.linear_acceleration = can_get_curvature ? computeCommandAccel() : 0;
  ccs.cmd.steering_angle = can_get_curvature ? atan(wheel_base_ * kappa) : 0;
...
double PurePursuitNode::computeCommandVelocity( const double &kappa) const
{
  if (param_flag_ == enumToInteger(Mode::dialog))
    return kmph2mps(const_velocity_);
  double v=sqrt(fabs(0.4*9.8*1/kappa));
  if(v>10)
  {
     ROS_INFO("***************" );
     return 10;

...
ROS_INFO("fen_mu :%lf", denominator);//加
ROS_INFO("curretn_pose_ :(%lf,%lf,%lf)", current_pose_.position.x, current_pose_.position.y, current_pose_.position.z);//加
  double numerator = 2 * calcRelativeCoordinate(target, current_pose_).y;
ROS_INFO("fen_zi :%lf", numerator);//加
  if (denominator != 0)
...
ROS_INFO("end_point :%d", next_waypoint);//加
ROS_INFO("start_point :%d", next_waypoint-1);//加
...
//ROS_INFO("b :%d ", b );//加
//ROS_INFO("c :%d ", c );//加
ROS_INFO("a,b,c : ( %lf , %lf , %lf)", a, b, c);//加
  if (!get_linear_flag)
    return false;
...
ROS_INFO("d :%lf", d);//加
  ...
  else
  {
    ...
    target2.z = current_pose_.position.z;
      ...
      ROS_INFO("target1 : ( %lf , %lf , %lf)", target1.x, target1.y, target1.z);//加
      ROS_INFO("target2 : ( %lf , %lf , %lf)", target2.x, target2.y, target2.z);//加
...
ROS_INFO("interval_start_end :%lf", interval);//加
    if (getPlaneDistance(target1, end) < interval)
    {
        ROS_INFO("result : target1");//加
      *next_target = target1;
      return true;
    }
    else if (getPlaneDistance(target2, end) < interval)
    {
        ROS_INFO("result : target2");//加
      *next_target = target2;
      return true;
    }
    else
    {
        ROS_INFO("result : false ");//加
      return false;
    }
  }
}
...
void PurePursuit::getNextWaypoint()
{
  ...
  {
    next_waypoint_number_ = -1;
ROS_INFO_STREAM("waypoint_flase ");
    return;
  }
  ...
  for (int i = 0; i < path_size; i++)
  {
    ...
    {
      next_waypoint_number_ = i;
ROS_INFO("next_waypoint_number :%d", next_waypoint_number_);
      return;
    }
  }
...
bool PurePursuit::canGetCurvature(double *output_kappa)
{
  ...
  getNextWaypoint();
ROS_INFO("getnextwaypoint_well :%d", next_waypoint_number_);//加
ROS_INFO("target1 : ( %lf , %lf , %lf)", current_waypoints_.at(next_waypoint_number_).pose.pose.position.x, current_waypoints_.at(next_waypoint_number_).pose.pose.position.y, current_waypoints_.at(next_waypoint_number_).pose.pose.position.z);//加
...
ROS_INFO("next_target_well");//加
  ...
  if (!interpolation)
  {
   ...
   // ROS_INFO("next_target : ( %lf , %lf , %lf)", next_target.x, next_target.y,next_target.z);
      ROS_INFO("next_target : ( %lf , %lf , %lf)", next_target_position_.x, next_target_position_.y,next_target_position_.z);//加
  *output_kappa = calcCurvature(next_target_position_);
  return true;
}

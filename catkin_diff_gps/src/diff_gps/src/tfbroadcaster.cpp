...
void tf_global::callback_gps(const NavSatFixConstPtr &gps) {
    ...
    if(origininitialized){
        ...
        double x_origin_gauss, y_origin_gauss;
...
bool tf_global::init_origin(){
    int counter = 0;
    double long_temp ,la_temp ;//存储临时经纬度数据
    ...
    do{
        ros::spinOnce();
        counter++;
        if(counter > 2){return false;}//循环三次不成功则返回false
    ...
    setOrigin(long_temp,la_temp);//将origin设为当前值（经纬度）
...
bool tf_global::imugpsready(){
    ...
    while((imuready == false) && (gpsready == false)){
        ...
        looprate.sleep();
        counter++;
        if(counter > 2 && imuready == false && gpsready == false){return false;}//若循环三次不成功，返回false
    };
    return true;
}
...
int main(int argc, char *argv[]) {
    ...
    tf_global tf_global;
    tf_global.run();
    std::cout<<"NO data from GPS/IMU"<<std::endl;//不会执行
    return 1;
}
